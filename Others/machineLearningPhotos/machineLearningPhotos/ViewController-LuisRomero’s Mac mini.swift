    ...
    func detectarImagenes(){
        ...
        let peticion = VNCoreMLRequest(model: model){(request, error) in
            ...
            DispatchQueue.main.async{
                self.dataLabel.text = "(\(primerResultado)"
            }
            
        }
        
        guard let ciimageForUser = CIImage(image: self.dataImage.image!) else {
            print("No se a podido crear la imagen CIImage a partir de un UIImage")
            return
        }
        
        //correr peticion
        let handler = VNImageRequestHandler(CIImage:ciimageForUser)
